...
## 更新履歴
* 2025/04/21
  - ツリービューのカテゴリフォルダ選択のショートカットキーの挙動
    + ショートカットキーで別カテゴリを選択した際、過去に選択していた場所からレジュームするよう変更
    + `Shift` - `Ctrl` - `<Num>` : 逆順に選択を切り替え
    + `Shift` - `Ctrl` - `0` : 選択をリドゥ (`Alt` - `0` と同じ)

* 2025/04/21
  - bugfix. 初回オープン時とカード更新時の処理の安定化

* 2025/04/19
  - コントロールボタンの変更
    + Tree View で選択中のディレクトリのカードのみを再読み込みするボタンを追加
    + Tree View の表示非表示を切り替えるボタンを削除
    + カード名の表示／非表示を切り替えるボタンを追加
    + ディレクトリ名表示ボタンが有効な場合にカード名をクリックすると、当該カードのディレクトリを選択するよう動作を変更
  - カードクリック動作の変更
    + 直前にクリックしたカードと同じカードをクリックした際、プロンプトの内容に変更があった場合は削除せずカードの内容で上書きするよう変更

* 2025/04/16
  - カード情報を更新した際にリフレッシュなしで当該カードの表示を更新するよう変更
  - PromptCardsタブの表示の高さがブラウザ内に納まるように変更
...
### キーボードショートカット
  - `Ctrl - 1 ~ 9` : カテゴリの1つ目から9つ目までを切り替えが行えます。連続して押すことでサブフォルダを順に移動します。
    + `Shift - Ctrl - 1 ~ 9` : 上記の逆順
  - `Ctrl - 0` / `Alt - 0` : カテゴリ切り替えの Undo / Redo が行えます
    + `Shift - Ctrl - 0` : でも Redo 可能
